...
describe("ScooterApp.registerUser(username, password, age)", () => {
  ...
  it.skip("throws an error if too young or already registered", () => {
    ...
    expect(() => {ScooterApp.registerUser('JohnSmith', 'CowBoyBeBop22', 23);}).toThrow('already registered');
...
describe("ScooterApp.createScooter(station)", () => {
  ...
  it.skip("throws an error if a station does not exist", () => {
    expect(() => {ScooterApp.createScooter('Wensleydale')}).toThrow('no such station');
  });
});
...
describe("ScooterApp.dockScooter(scooter, station)", () => {
  ...
  it.skip("throws an error if a station does not exist", () => {
    s1 = new Scooter(null);
    expect(() => {ScooterApp.dockScooter(s1, 'Wensleydale')}).toThrow('no such station');
  });

  it.skip("throws an error if a scooter is already at a station", () => {
    s1 = new Scooter('Woodmere');
    expect(() => {ScooterApp.dockScooter(s1, 'Woodmere')}).toThrow('scooter already at station');
  });
});
...
describe("ScooterApp.rentScooter(scooter, user)", () => {
  ...
  it.skip("throws an error if a scooter is already rented", () => {
    u1 = new User('JohnSmith', 'CowBoyBeBop22', 23);
    s1 = new Scooter(null);
    expect(() => {ScooterApp.rentScooter(s1, u1)}).toThrow('scooter already rented');
  });
});
